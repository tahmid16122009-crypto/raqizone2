...
<div class="page">
  ...
  <div class="bnr-wrap d-mobile" id="mBnrWrap">
    ...
    <?php endif; ?>
    <?php else: ?><div class="bnr-e"><span>🛍️</span><p><?= htmlspecialchars($cfg['home_tagline'] ?? '') ?></p></div><?php endif; ?>
  </div>
  ...
  <div class="sw" style="padding:8px 14px">
    <div style="display:flex;gap:7px;align-items:center">
      ...
      <button onclick="doSearch()" style="padding:9px 13px;background:linear-gradient(135deg,var(--g),var(--gd));color:var(--k);border:none;border-radius:50px;cursor:pointer;font-weight:700;font-family:inherit;font-size:.82rem;flex-shrink:0">🔍</button>
    </div>
    ...
    <div id="fp" style="display:none;margin-top:10px;padding:12px;background:var(--k3);border-radius:var(--r);border:1px solid var(--bdr2)">
      <?php if ($categories): ?>
      <p style="font-size:.72rem;color:var(--gray);margin-bottom:8px;font-weight:600;text-transform:uppercase">📂 Category</p>
      ...
      <?php endif; ?>
      <p style="font-size:.72rem;color:var(--gray);margin-bottom:8px;font-weight:600;text-transform:uppercase">👥 Gender</p>
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        <button type="button" class="fcat<?= (!$gender||$gender==='all')?' fcat-on':'' ?>" onclick="qGender('')">🌟 All</button>
        <button type="button" class="fcat<?= $gender==='male'?' fcat-on':'' ?>" onclick="qGender('male')">👨 Male</button>
        <button type="button" class="fcat<?= $gender==='female'?' fcat-on':'' ?>" onclick="qGender('female')">👩 Female</button>
      </div>
    </div>
  </div>
  ...
  <div style="padding:5px 14px;display:flex;gap:6px;flex-wrap:wrap">
    ...
    <?php if ($gender && $gender !== 'all'): ?><a href="/home<?= $cat?'?cat='.$cat:'' ?>" style="display:inline-flex;align-items:center;gap:4px;padding:3px 10px;background:var(--gl);color:var(--g);border-radius:50px;font-size:.74rem;font-weight:600;text-decoration:none"><?= $gender==='male'?'👨':'👩' ?> <?= $gender ?> ✕</a><?php endif; ?>
  </div>
  <?php endif; ?>
    ...
    <div class="pg">
      ...
      <a href="/product/<?= $p['id'] ?>" class="pc">
        <div class="pci">
          ...
          <?php if ($p['gender']==='male'): ?><span class="gender-badge">👨</span><?php elseif ($p['gender']==='female'): ?><span class="gender-badge">👩</span><?php endif; ?>
        </div>
        <div class="pin">
...
<div class="overlay" id="ls">
  <div class="modal" style="position:relative">
    ...
    <div class="mt"><span class="ic">🔐</span><h3 data-bn="লগিন করুন" data-en="Login">Login</h3></div>
...
function renderSug(items,q){
  ...
  items.forEach(function(item){
    ...
    html+='<div class="sug-item" onclick="window.location.href=\'/product/\'+'+item.id+'">'+
      (item.thumb?'<img src="'+item.thumb+'" style="width:38px;height:38px;object-fit:cover;border-radius:6px;flex-shrink:0">':'<div style="width:38px;height:38px;border-radius:6px;background:var(--k3);flex-shrink:0;display:flex;align-items:center;justify-content:center">🛍️</div>')+
