...
.lp-bg-wrap::before,.lp-bg-wrap::after{
...
@keyframes lpFloat{0%,100%{transform:translate(0,0) scale(1);}50%{transform:translate(30px,-25px) scale(1.12);}}
...
body.has-bg-img .lp-bg-wrap::after{
  ...
  filter:blur(18px) saturate(1.1);background:inherit;opacity:1;animation:none;
  background-image:var(--bg-img);background-size:cover;background-position:center;
}
...
.lp-title{opacity:0;transform:translateY(10px);animation:lpFadeUp .55s ease .5s forwards;}
.lp-sub{opacity:0;transform:translateY(10px);animation:lpFadeUp .55s ease .6s forwards;}
@keyframes lpFadeUp{to{opacity:1;transform:translateY(0);}}
.lp-lang{opacity:0;transform:translateY(10px);animation:lpFadeUp .55s ease .7s forwards;}
.lp-btns>*{opacity:0;transform:translateY(14px);animation:lpFadeUp .55s ease forwards;}
...
<body class="<?= $theme ?><?php if (!empty($cfg['mobile_bg_image'])): ?> has-bg-img<?php endif; ?>"
      id="bodyRoot"
      <?php if (!empty($cfg['mobile_bg_image'])): ?>style="--bg-img:url('<?= htmlspecialchars($cfg['mobile_bg_image']) ?>')"<?php endif; ?>>
...
<div class="wp" style="position:relative;z-index:2;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px">
  <div class="lp-card">
    ...
    <div class="wbtns lp-btns">
      <button class="bg" onclick="om('ml')">
        <svg viewBox="0 0 24 24" style="width:18px;height:18px;fill:currentColor;flex-shrink:0"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 10.99h7c-.53 4.12-3.28 7.79-7 8.94V12H5V6.3l7-3.11v8.8z"/></svg>
        <span data-bn="লগিন করুন" data-en="Login">Login</span>
      </button>
      <button class="bo" onclick="om('mr')">
        <svg viewBox="0 0 24 24" style="width:18px;height:18px;fill:currentColor;flex-shrink:0"><path d="M15 14c-2.67 0-8 1.33-8 4v2h16v-2c0-2.67-5.33-4-8-4zm-8.94-6H4v2h2.06v2.06h2V10H10V8H8.06V5.94h-2V8zM15 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4z"/></svg>
        <span data-bn="অ্যাকাউন্ট খুলুন" data-en="Create Account">Create Account</span>
      </button>
...
<div class="overlay" id="ml">
  <div class="modal" style="position:relative">
    ...
    <div class="mt">
      <span class="ic"><svg viewBox="0 0 24 24" style="width:32px;height:32px;fill:var(--g)"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 10.99h7c-.53 4.12-3.28 7.79-7 8.94V12H5V6.3l7-3.11v8.8z"/></svg></span>
      <h3 data-bn="লগিন করুন" data-en="Login">Login</h3>
    </div>
    <form action="/auth/login" method="POST" class="fs">
      <input type="hidden" name="next" value="/home">
      <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token) ?>">
      <div class="fd">
        <label data-bn="আপনার নাম" data-en="Your Name">Name</label>
        <input type="text" name="name" class="inp" placeholder="Full name" required autocomplete="name">
      </div>
      <div class="fd">
        <label data-bn="মোবাইল নম্বর" data-en="Mobile Number">Mobile</label>
        <input type="tel" name="mobile" class="inp" placeholder="01XXXXXXXXX" required autocomplete="tel">
      </div>
...
<div class="overlay" id="mr">
  <div class="modal" style="position:relative">
    ...
    <div class="mt">
      <span class="ic"><svg viewBox="0 0 24 24" style="width:32px;height:32px;fill:var(--g)"><path d="M15 14c-2.67 0-8 1.33-8 4v2h16v-2c0-2.67-5.33-4-8-4zm-8.94-6H4v2h2.06v2.06h2V10H10V8H8.06V5.94h-2V8zM15 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4z"/></svg></span>
      <h3 data-bn="অ্যাকাউন্ট খুলুন" data-en="Create Account">Create Account</h3>
    </div>
    <form action="/auth/login" method="POST" class="fs">
      <input type="hidden" name="next" value="/home">
      <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token) ?>">
