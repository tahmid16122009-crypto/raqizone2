...
<div class="aph">
  ...
  <?php endif; ?>
  <h1>⚙️ Settings<?= $section ? ' — '.ucfirst($section) : '' ?></h1>
  <?php if ($saved): ?>
  <div style="background:rgba(76,175,80,.12);color:#4CAF50;border:1px solid rgba(76,175,80,.25);padding:8px 14px;border-radius:8px;font-size:.84rem;margin-top:8px">✅ Saved!</div>
  <?php endif; ?>
</div>
...
<div class="smenu-grid">
  <?php
  $menus = [
    ['logo',       '🖼️', 'Logo & Favicon',     'Site logo, favicon, name'],
    ['seo',        '🔍', 'SEO / Google',        'Meta description, keywords'],
    ['theme',      '🎨', 'UI Theme',             'Color theme selection'],
    ['banner',     '📸', 'Banner & Background',  'Banners, BG images'],
    ['categories', '📂', 'Categories',           'Product categories'],
    ['identity',   '🏪', 'Site Identity',        'Title, taglines, footer'],
    ['delivery',   '🚚', 'Delivery & Payment',   'Charges, payment methods'],
    ['contact',    '📞', 'Contact Us',           'Phone, WhatsApp, Email'],
    ['social',     '📱', 'Social Media',         'Links & icons'],
    ['info',       '📋', 'Info & Terms',         'About, terms, policy'],
  ];
...
<form action="/admin/edit-ui?section=logo" method="POST" enctype="multipart/form-data" class="aform">
  ...
  <div class="afs">
    ...
    <div class="fr2">
      <div class="frow">
        ...
        <div class="aup" onclick="document.getElementById('logoInp').click()"><div class="ui">🖼️</div><p>Upload Logo</p></div>
      ...
      <div class="frow">
        ...
        <?php if (!empty($cfg['site_favicon_ext'])): ?><p style="font-size:.74rem;color:#4CAF50;margin-bottom:6px">✅ Favicon set</p><?php endif; ?>
        <div class="aup" onclick="document.getElementById('favInp').click()"><div class="ui">🔖</div><p>Upload Favicon</p></div>
  ...
  <div class="afact"><button type="submit" class="abg">✅ Save</button></div>
</form>
...
<form action="/admin/edit-ui?section=theme" method="POST" class="aform">
  ...
  <div class="afact"><button type="submit" class="abg">✅ Save Theme</button></div>
</form>
<script>
...
<form action="/admin/edit-ui?section=banner" method="POST" enctype="multipart/form-data" class="aform">
  ...
  <div class="afs">
    ...
    <div class="aup" onclick="document.getElementById('mBnrInp').click()" style="margin-top:8px"><div class="ui">📷</div><p>Add mobile banners</p></div>
  ...
  <div class="afs">
    ...
    <div class="aup" onclick="document.getElementById('dBnrInp').click()" style="margin-top:8px"><div class="ui">🖥️</div><p>Add desktop banners</p></div>
    ...
    <div class="frow" style="margin-top:12px">
      ...
      <select name="banner_animation" class="ai">
        <?php foreach(['slide'=>'➡️ Slide','fade'=>'✨ Fade','zoom'=>'🔍 Zoom','flip'=>'🔄 Flip'] as $v=>$l): ?>
  ...
  <div class="afs">
    ...
    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-bottom:8px">
      ...
      <span style="color:#F44336;font-size:.82rem;font-weight:600">✕ Remove mobile BG</span>
    </label>
    <?php endif; ?>
    <div class="aup" onclick="document.getElementById('mBgInp').click()"><div class="ui">🖼️</div><p>Upload Mobile BG</p></div>
  ...
  <div class="afs">
    ...
    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-bottom:8px">
      ...
      <span style="color:#F44336;font-size:.82rem;font-weight:600">✕ Remove desktop BG</span>
    </label>
    <?php endif; ?>
    <div class="aup" onclick="document.getElementById('dBgInp').click()"><div class="ui">🖥️</div><p>Upload Desktop BG</p></div>
...
function renderCats(){const list=document.getElementById('catList');list.innerHTML='';cats.forEach((cat,i)=>{const d=document.createElement('div');d.style.cssText='display:flex;align-items:center;gap:8px;background:var(--ak3);border:1px solid var(--abdr);border-radius:8px;padding:8px 12px';d.innerHTML='<span style="flex:1;font-size:.86rem">📂 '+cat+'</span><button type="button" class="idl" onclick="rmCat('+i+')">✕</button>';list.appendChild(d);});document.getElementById('catsJson').value=JSON.stringify(cats);}
  ...
  <div class="afs">
    ...
    <div class="frow">
      ...
      <select name="payment_options" class="ai" id="payOptSel" onchange="showPS()">
        <?php foreach(['cod'=>'💵 COD Only','delivery_only'=>'🏦 Delivery Charge Online','full'=>'💳 Full Payment','all'=>'🔀 All Options'] as $v=>$l): ?>
...
<form action="/admin/edit-ui?section=contact" method="POST" enctype="multipart/form-data" class="aform">
      ...
      <div class="contact-row">
        <div style="display:flex;gap:9px;align-items:center">
          <div style="flex-shrink:0;cursor:pointer" onclick="document.getElementById('cpf<?= $i ?>').click()">
            ...
            <?php else: ?>
            <div id="cpimg<?= $i ?>" style="width:46px;height:46px;border-radius:50%;background:var(--ak2);border:2px dashed var(--abdr);display:flex;align-items:center;justify-content:center;font-size:1.2rem">👤</div>
            <?php endif; ?>
      ...
      <div class="contact-row">
        <div style="display:flex;gap:9px;align-items:center">
          <div style="flex-shrink:0;cursor:pointer" onclick="document.getElementById('wpf<?= $i ?>').click()">
            ...
            <?php else: ?>
            <div id="wpimg<?= $i ?>" style="width:46px;height:46px;border-radius:50%;background:var(--ak2);border:2px dashed var(--abdr);display:flex;align-items:center;justify-content:center;font-size:1.2rem">💬</div>
            <?php endif; ?>
      ...
      <div class="contact-row">
        <div style="display:flex;gap:9px;align-items:center">
          <div style="flex-shrink:0;cursor:pointer" onclick="document.getElementById('emf<?= $i ?>').click()">
            ...
            <?php else: ?>
            <div id="emimg<?= $i ?>" style="width:46px;height:46px;border-radius:50%;background:var(--ak2);border:2px dashed var(--abdr);display:flex;align-items:center;justify-content:center;font-size:1.2rem">📧</div>
            <?php endif; ?>
  ...
  <div class="afact"><button type="submit" class="abg">✅ Save All</button></div>
</form>
      ...
      <div class="contact-row">
        <div style="display:flex;gap:9px;align-items:center">
          <div style="flex-shrink:0;cursor:pointer" onclick="document.getElementById('smf<?= $i ?>').click()">
            ...
            <?php else: ?>
            <div id="smimg<?= $i ?>" style="width:46px;height:46px;border-radius:10px;background:var(--ak2);border:2px dashed var(--abdr);display:flex;align-items:center;justify-content:center;font-size:1.3rem">📱</div>
            <?php endif; ?>
